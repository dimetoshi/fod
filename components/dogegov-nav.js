// DOGEGOV Navigation Component with Active Page Detection
class DogeGovNav {
  ...
  // NEW: Method to detect and set active button
  setActiveButton() {
    const currentUrl = window.location.href;
    const currentPath = window.location.pathname;
    
    // Get all nav buttons (desktop and mobile)
    ...
    [...desktopButtons, ...mobileButtons].forEach((button, index) => {
      ...
      // Check for exact match or if current URL contains the button's path
  ...
  // NEW: Helper method to determine if a link is active
  ...
  attachEventListeners() {
    ...
    // NEW: Update active state when navigating via AJAX/SPA
    window.addEventListener('popstate', () => {
      this.setActiveButton();
    });
  }
  
  // NEW: Public method to manually set active button (useful for SPAs)
  ...
  // NEW: Public method to refresh active state
  refreshActiveState() {
    this.setActiveButton();
  }
